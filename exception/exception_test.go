...
func TestNewExceptionWithAllFields(t *testing.T) {
	...
	assert.Equal(t, "E123", ex.Code)
	assert.Equal(t, "DatabaseError", ex.Name)
...
func TestErrorsAs(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, "ERR42", ex.Code)
	assert.Equal(t, "BadInput", ex.Name)
	assert.Equal(t, cause, ex.Cause)
}
